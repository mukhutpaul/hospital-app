...
export default function SortiesTable({
  items,
}: Props) {
  ...
  const types = useMemo(() => {
    ...
    items.forEach((sortie) => {
      if (sortie.type) {
        values.add(String(sortie.type));
      }
    });
  ...
  const sortiesAujourdhui = items.filter((sortie) => {
  ...
  const patients = new Set(
    items
      .map((sortie) => sortie.patient?.id)
      .filter(Boolean)
  ).size;

  const typesUtilises = new Set(
    items
      .map((sortie) => sortie.type)
      .filter(Boolean)
  ).size;
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((sortie) => {
      const patientName = [
        sortie.patient?.nom,
        sortie.patient?.postNom,
        sortie.patient?.postnom,
        sortie.patient?.prenom,
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
      ...
      const searchableText = [
        patientName,
        sortie.hospitalisation?.numero,
        sortie.type,
        sortie.diagnosticFinal,
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  function getPatientName(sortie: any) {
    return [
      sortie.patient?.nom,
      sortie.patient?.postNom,
      sortie.patient?.postnom,
        ...
        <div className="border-b border-base-200 p-5">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <label className="input input-bordered flex w-full items-center gap-2 lg:max-w-md">
              ...
              <input
                type="search"
                placeholder="Patient, hospitalisation, diagnostic..."
                value={search}
                onChange={(e) =>
                    ...
                    >
                      ...
                      <td>
                        ...
                        <div className="flex justify-end gap-1">
                          ...
                          <Link
                            href={`/hospitalisation/sorties/${s.id}`}
