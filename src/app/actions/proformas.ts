...
export async function getPatientPrestations(
  ...
  try {
    ...
    const anciennesLignes =
      await prisma.proformaLigne.findMany({
        where: {
          proforma: {
            patientId,
          },
    ...
    const labDejaPris = new Set<number>(
      anciennesLignes
        .map((ligne) => ligne.demandeLaboratoireId)
        .filter(
          (id): id is number =>
            id !== null
        )
    ...
    const imagerieDejaPrise = new Set<number>(
      anciennesLignes
        .map((ligne) => ligne.demandeImagerieId)
        .filter(
          (id): id is number =>
            id !== null
        )
    ...
    const pharmacieDejaPrise = new Set<number>(
      anciennesLignes
        .map((ligne) => ligne.dispensationId)
        .filter(
          (id): id is number =>
            id !== null
        )
    ...
    const hospitalisationDejaPrise = new Set<number>(
      anciennesLignes
        .map((ligne) => ligne.hospitalisationId)
        .filter(
          (id): id is number =>
            id !== null
        )
    ...
    const consultationsDejaPrises = new Set<number>(
      anciennesLignes
        .map((ligne) => ligne.consultationId)
        .filter(
          (id): id is number =>
            id !== null
        )
    ...
    return {
      ...
      message:
        "Prestations du patient récupérées.",
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        message:
          "Un des identifiants fournis est invalide.",
      };
    }
    ...
    if (
      ...
      !(
        input.dateExpiration instanceof
        Date
      )
    ) {
      return {
    ...
    if (
      ...
      return {
        success: false,
        message:
          "La proforma doit contenir au moins une ligne.",
      };
    }

    const lignes =
      normalizeProformaLignes(
        input.lignes,
        {
          consultationId:
            input.consultationId,
    ...
    if (lignes.length === 0) {
      return {
        success: false,
        message:
          "Aucune ligne de prestation valide.",
      };
    }
...
export async function createProformaFromPrestations(
  input: {
    patientId: number;
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        message:
          "Les prestations sélectionnées sont invalides.",
      };
    }
...
export async function createProformaPatient(
  input: {
    patientId: number;
  ...
  try {
    ...
    const lignesAutomatiques =
      prestations.data.lignes || [];

    const lignesManuelles =
      input.lignesSupplementaires ||
      [];
    ...
    if (lignes.length === 0) {
      return {
        success: false,
        message:
          "Aucune prestation facturable trouvée pour ce patient.",
      };
    }
    ...
    return {
      ...
      message:
        "Erreur lors de la génération automatique de la proforma.",
    };
  }
}
...
export async function validerProforma(
  id: number
): Promise<ActionResult> {
  try {
    if (
    ...
    if (
      ...
      return {
        success: true,
        message:
          "Cette proforma est déjà validée.",
        data: proforma,
      };
    }

    if (
      proforma.dateExpiration &&
      proforma.dateExpiration <
        new Date()
    ) {
      return {
  ...
  try {
    if (
      ...
      return {
        success: false,
        message:
          "Identifiant de proforma invalide.",
      };
    }
          ...
          const nouvelleFacture =
            await tx.facture.create({
              data: {
                ...
                consultationId:
                  proforma.consultationId,

                admissionId:
                  proforma.admissionId,

                hospitalisationId:
                  proforma.hospitalisationId,
    ...
    revalidatePath(
      `/facturation/proformas/${proformaId}`
    );
    ...
    return {
      ...
      message:
        "Erreur lors de la conversion de la proforma en facture.",
    };
  }
}
...
export async function getPatientConsultations(
  ...
  try {
    ...
    const pharmacieParConsultation =
      new Map<number, any[]>();
    ...
    for (const dispensation of dispensations) {
      ...
      const liste =
        pharmacieParConsultation.get(
          consultationId
        ) ?? [];

      liste.push(dispensation);

      pharmacieParConsultation.set(
        consultationId,
        liste
      );
    }
    ...
    const consultationsAvecPharmacie =
      consultations.map(
        (consultation) => {
          const pharmacie =
            pharmacieParConsultation.get(
          ...
          return {
            ...
            prestationsPharmacie:
              pharmacie.flatMap(
                (dispensation) =>
                  dispensation.lignes.map(
                    (ligne: any) => ({
                      id: ligne.id,
